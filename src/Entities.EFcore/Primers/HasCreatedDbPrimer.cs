...
/// An update that does not write <c>Created</c> is left alone — a soft delete writes only its own columns, and a stub's
/// original is not the stored value.<br />
...
public class HasCreatedDbPrimer : EntityPrimerBase<IHasCreated>
{
    ...
    {
        if (entry.State == EntityState.Modified && !entry.Property(nameof(entity.Created)).IsModified)
        {
